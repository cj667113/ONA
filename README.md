...
The gateway dashboard streams updates in place about every 30 seconds with CPU, memory, conntrack, NAT port, connection, packet, and network throughput statistics. Dashboard history is stored in `/ONA/flask/instance/dashboard_history.json` by default so recent metrics are available after browser refreshes, new logins, and container restarts when the instance directory is mounted as a persistent volume. Set `ONA_DASHBOARD_HISTORY_FILE` to override the storage path.
